...
void* SendToSrv(void *arg)
{
	...
	while (true) {
		DHT11 sensor;
		data TheData = sensor.dht11_read();
		//cout<<"T:"<<TheData.temperature<<"H:"<<TheData.humidity<<endl;
		sprintf(sendline,"%d:T:%d,H:%d",id++,TheData.temperature,TheData.humidity);
		//cout<<sendline<<endl;
		int len = write(connfd, sendline, MAXLINE);
		if (len > 0) {
			printf("SendData: %d - %s\n", len, sendline);
			...
			close(connfd);
			printf("SendData:[fd=%d] error[%d]: %s\n", (int)connfd,errno,strerror(errno));
		}
		sleep(1);
	}
}
...
{
	char * servInetAddr = "192.168.1.101";
	int servPort = 6888;
	//char buf[MAXLINE];
	//int connfd;
	//struct sockaddr_in servaddr;
